<?php
        //la question pour un moderateur connecter
        $vide=true;
        echo '<!--discussion-->
        ...
        try{
         ...
         $reponse->execute(array($_GET["domaine"],$_GET["id_quest"]));
            while ($donnees = $reponse->fetch()){
                    $vide=false; //si la question n'est pas supprimé ou il n'ya pas de question
                    echo "<!--Discussion-->
                    <div class='col s12 message'>
                        ...
                        //le boutton signaler question
                          echo "<form action='../part/signalerQuest.php' method='post' class='col s1 right-align'>
                                <input type='submit' title='Signaler' style='font-size:20px; border:none; background-color:transparent' value='&#9888;'>
        ...
        //la liste des réponses associés à la question
        try{
         $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
         $bdd = new PDO('mysql:host=localhost;dbname=id1227213_mydb', 'id1227213_abc', '', $pdo_options);
         $bdd->exec('set names utf8');
