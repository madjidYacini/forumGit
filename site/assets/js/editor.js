		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											"icon": "fa fa-font",
											"tooltip": "Couleur du texte/remplissage",
											"commandname":null,
						     ...
						     'strikeout': { "text": "Strikeout",
											...
											"tooltip": "Frapper à travers",
											"commandname":"strikeThrough",
											"custom":null },

						     'ol'		: { "text": "N",
											"icon": "fa fa-list-ol",
											"tooltip": "Insérer/supprimer une liste numérotée",
											"commandname":"insertorderedlist",
											"custom":null },

						     'ul'		: { "text": "Bullet",
											"icon": "fa fa-list-ul",
											"tooltip": "Insérer/Supprimer une liste à puces",
						     ...
						     'c_align'	: { "text": "centeralig",
											"icon": "fa fa-align-center",
											"tooltip": "Aligner au centrer",
						   ...
						   'insert_link': { "modal": true,
											...
											"onSave":function(){
												...
												if(targetURL ==''){
													methods.showMessage.apply(editorObj,["errMsg","Veillez entrer un URL"]);
													return false;
												}
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["errMsg","Entrer un URL valide"]);
													return false;
												}
												if(range=='' && targetText==''){
						...
						'insert_table'	: { "modal": true,
											...
											"onSave": function(){
												...
												if(!tblRows.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","Lignes doit être un nombre positif"]);
													return false;
												}
												if(!tblColumns.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","colonnes doit être un nombre positif"]);
													return false;
												}
												if(tblWidth!="" && !tblWidth.match(cssReg)){
													methods.showMessage.apply(this,["tblErrMsg","Veuillez entrer un nombre positif avec ou sans unité de mesure CSS valide (px,em,ex,%,in,cm,mm,pt,pc)"]);
													return false;
												}
												...
												if(tblBorder!="" && !tblBorder.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","La taille de la bordure doit être un nombre positif"]);
													return false;
												}
												if(tblCellspacing!="" && !tblCellspacing.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","L'espacement des cellules doit être un nombre positif"]);
													return false;
												}
												if(tblCellpadding!="" && !tblCellpadding.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","Rembourrage cellulaire doit être un nombre positif"]);
													return false;
												}
						   ...
						   'outdent'	: { "text": "Outdent",
							...
							'splchars'	: { "text": "S",
											"icon": "fa fa-asterisk",
											"tooltip": "Insérer un caractère spéciale",
											"commandname":null,
											"custom":function(button){
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").html('<div class="label">'+'Mots : '+wordCount+'</div>');
					$(editor_Content).data("statusBar").append('<div class="label">'+'Caractère : '+charCount+'</div>');
            	});
	        }
								            	...
								            	class:"modal-dialog"
							            			...
							            			class:"modal-content"
									            		...
									            		class:"modal-footer"
								                			...
								                			type:"button",
								                			class:"btn btn-success",
																		}).html('Fait').mousedown(function(e){
								                			e.preventDefault();
